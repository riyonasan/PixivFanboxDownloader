...
- 「重複するファイルをダウンロードしない」をオンにすると、Eagle登録でも拡張機能のIndexedDB履歴にある同じ記事・ファイルをスキップします。履歴にないファイルはEagleの記事フォルダ内で名前と拡張子を照合し、既存の項目が見つかれば履歴に記録してスキップします。以後は履歴だけで判定します。通常ダウンロードの履歴とは区別されます。名前やフォルダ構成を変更した項目は照合できず、ファイルの中身までは比較しません。Eagleへの登録依頼を受け付けた時点で履歴に残すため、Eagle側で後から取り込みに失敗した場合は、この設定を一時的にオフにして再登録してください。
