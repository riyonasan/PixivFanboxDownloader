...
- 「重複するファイルをダウンロードしない」をオンにすると、Eagle登録でも拡張機能のIndexedDB履歴にある同じ記事・ファイルをスキップします。通常ダウンロードの履歴とは区別されます。Eagleへの登録依頼を受け付けた時点で履歴に残すため、Eagle側で後から取り込みに失敗した場合は、この設定を一時的にオフにして再登録してください。更新前にEagleへ登録したファイルは履歴に含まれません。
